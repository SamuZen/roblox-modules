...
`CombatPositioning.Delta` accepts an optional final `intent` displacement, replacing only the seek term before crowd steering. `Navigation.new` supplies this intent with Direct/Avoid/Path recovery, injected `Probe`, `ComputePath`, `Position`, `Visible`, `Clock` and optional `BeginFrame`/`Config`. Call `Step(now)` once per server heartbeat before moving records, `Delta` only during permitted locomotion, `Observe` with the final rejection reason, `Forget` on inactive locomotion/removal and `Destroy` on shutdown. `Probe` returns true/false/nil (clear/blocked/deferred); budget deferral must never count as a wall. Path computation runs in bounded asynchronous workers, guarded against stale target/encounter/action results. The host retains collision, ground, bounds and action authority; navigation never executes jumps or changes a combat target. Coordinator removal clears `record.Navigation` to invalidate outstanding requests.
