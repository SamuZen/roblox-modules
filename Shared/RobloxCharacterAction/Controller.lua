...
function Controller.new(character: Model, actionsFolder: Folder, setActionRemote: RemoteEvent)
	-- Characters are not replicated atomically so we need to wait for children to replicate
	local humanoid = character:WaitForChild("Humanoid")
	local root = character:WaitForChild("HumanoidRootPart")

	local self = {
        setActionRemote = setActionRemote,
        actions = loadModules(actionsFolder),
		character = character,
		humanoid = humanoid,
		root = root,
