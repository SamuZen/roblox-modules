...
function Telegraph.new(parent,range,halfAngle)
    local model=Instance.new("Model")
    model.Name,model.Parent="AttackTelegraph",parent
    local self=setmetatable({Model=model,Parts={},Range=range,Angle=math.rad(halfAngle)},Telegraph)
    for index=1,14 do
...
function Telegraph:Show(frame,progress,strike)
    ...
    for index,part in self.Parts do
        local a,b
        if index<=12 then a,b=point(-self.Angle+(index-1)*self.Angle/6),point(-self.Angle+index*self.Angle/6)
